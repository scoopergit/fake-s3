...
module FakeS3
  class CLI < Thor
    ...
    method_option :origin, :type => :string, :aliases => '-o', :required => false
    ...
    def server
      ...
      puts "Loading FakeS3 with #{root} on port #{options[:port]} with hostname #{hostname} for origin #{origin}"
      server = FakeS3::Server.new(address,options[:port],origin,store,hostname,ssl_cert_path,ssl_key_path)
      server.serve
    end
